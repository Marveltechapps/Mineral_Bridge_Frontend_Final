...
import { resolveOrderAmountForDisplay, resolveLegacyAmountNumber } from './orderPricingDisplay';
...
/** Whether order history / list cards should show an amount row (buy and sell). */
export function shouldShowOrderAmountOnCard(order) {
  if (hasSavedPriceBreakdown(order)) return true;
  const display = getPrimaryTotalDisplayString(order);
  return Boolean(display && display !== '—' && display !== 'Price pending');
}
...
export function getPrimaryTotalDisplayString(order) {
  ...
  const resolved = resolveOrderAmountForDisplay(order);
  if (resolved.mode !== 'pending') return resolved.display;
  const legacy = resolveLegacyAmountNumber(order);
  if (legacy != null && legacy > 0) return formatLegacyMoney(legacy);
