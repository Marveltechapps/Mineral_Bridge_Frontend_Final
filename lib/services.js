 ...
 */
import { getApiBase, fetchWithAuth, fetchMultipart, fetchPublic, readApiJsonBody } from './api';
...
export async function sendOtp(body) {
  ...
  if (!res.ok) throw new Error(data.message || data.error || `Failed to send OTP (${res.status})`);
  return data;
}
...
export async function registerOrLogin(body) {
  const res = await fetchPublic('/api/auth/register-or-login', {
    method: 'POST',
    body: JSON.stringify(body),
  });
  const data = await res.json().catch(() => ({}));
...
function resolveImageMimeType(uri, mimeType = 'image/jpeg') {
  const fromPicker = String(mimeType || '').toLowerCase().trim();
  if (fromPicker && fromPicker !== 'image/jpeg') return fromPicker;
  const match = String(uri || '').match(/\.(jpe?g|png|webp|heic|heif)$/i);
  if (!match) return fromPicker || 'image/jpeg';
...
export async function uploadAvatar(uri, mimeType = 'image/jpeg') {
  try {
    const resolvedMime = resolveImageMimeType(uri, mimeType);
    const ext = imageExtFromMime(resolvedMime);
    const form = new FormData();
    form.append('avatar', { uri, name: `avatar${ext}`, type: resolvedMime });
...
// —— Artisanal (auth)
/** Check if logged-in user can access artisanal screens (open to all countries). */
export async function getArtisanalCanAccess() {
  const res = await fetchWithAuth('/api/artisanal/can-access');
  if (!res.ok) return { canAccess: true, isEligible: true, country: null };
  return res.json();
}
